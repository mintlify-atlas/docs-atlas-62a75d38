...
description: "Deploy RomM using Docker Compose, configure your database, mount your ROM library, and scan your first games in under 10 minutes."
---
...
<Steps>
  ...
  <Step title="Complete the setup wizard">
    ...
    <Note>
      The setup wizard only runs once. If you need to reset it (for example, after a fresh database), you can set `DISABLE_SETUP_WIZARD=false` in your environment and restart the container. To skip the wizard entirely on purpose, set `DISABLE_SETUP_WIZARD=true` before the first boot.
    </Note>
